...
static int
strings_index_of_largest (int n, char *strings[])
{
  ...
  for (i = 1; i < n; i++)
    {
      ...
      if (strcmp (strings[guess], strings[i]) < 0)
        {
          // Update our guess
          guess = i;
...
int
strings_sort (int n, char *strings[])
{
  int i;
  for (i = n-1; i > 0; i--)
    {  
      strings_swap (i, strings, i, strings_index_of_largest (i+1, strings));
    } // for i
  return 1;     // Success
} // strings_sort
